# Reserve Bank of New Zealand Exchange Rates API — rbnz-exchange-rate
...
The latest Reserve Bank of New Zealand table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/rbnz?source=NZD&target=USD"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/rbnz').then((x) => x.json());
...
Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/rbnz-exchange-rate`](https://www.npmjs.com/package/@allratestoday/rbnz-exchange-rate) — same code, same versions.
...
The Reserve Bank of New Zealand publishes daily wholesale exchange rates for the New Zealand dollar against 18 currencies (its B1 statistical series), alongside the trade-weighted index, each business day at around 3pm New Zealand time.

- Publisher's own page: [B1 — exchange rates and TWI](https://www.rbnz.govt.nz/statistics/series/exchange-and-interest-rates/exchange-rates-and-the-trade-weighted-index) · [www.rbnz.govt.nz](https://www.rbnz.govt.nz)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Reserve Bank of New Zealand rates page](https://allratestoday.com/central-bank-rates-api/rbnz/)
...
- Reserve Bank of New Zealand quotes **foreign currency per 1 NZD** (e.g. `base: "NZD", quote: "USD"` means USD per one NZD).
...
Loading the official Reserve Bank of New Zealand rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/rbnz/latest?format=xml&api_key=art_live_..."
```
...
- [Package docs on the site](https://allratestoday.com/docs/sdk/rbnz-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
